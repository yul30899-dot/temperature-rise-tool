      ...
      try {
        ...
        if (chartFile) {
          ...
          channels.forEach((ch, idx) => {
              ...
              serNodes += `<${p}ser><${p}idx val="${idx}"/><${p}order val="${idx}"/><${p}tx><${p}strRef><${p}f>原始数据!${colLetter}$1</${p}f><${p}strCache><${p}ptCount val="1"/><${p}pt idx="0"><${p}v>CH${ch.id}</${p}v></${p}pt></${p}strCache></${p}strRef></${p}tx><${p}spPr><a:ln w="28575" xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main"/></${p}spPr><${p}marker><${p}symbol val="none"/></${p}marker><${p}cat><${p}strRef><${p}f>原始数据!$A$2:$A${maxRows}</${p}f><${p}strCache><${p}ptCount val="${maxRows-1}"/></${p}strCache></${p}strRef></${p}cat><${p}val><${p}numRef><${p}f>原始数据!${colLetter}$2:${colLetter}${maxRows}</${p}f><${p}numCache><${p}formatCode>General</${p}formatCode><${p}ptCount val="${maxRows-1}"/></${p}numCache></${p}numRef></${p}val><${p}smooth val="1"/></${p}ser>`;
          });
